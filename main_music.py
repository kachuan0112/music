...
class Song:
    ...
    def create_embed(self):
        embed = embed = (discord.Embed(title='Now Playing :notes:', description='**```css\n{0.source.title}\n```**'.format(self), color=discord.Color.blurple())
                .add_field(name='Duration', value=f"▬▬▬▬▬|▬▬▬▬▬▬▬▬▬ `00:00:00/{self.source.duration}`",inline=False)
                .add_field(name='Requested by', value=f"--{self.requester.mention}--")
                .add_field(name='Uploader', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self))
                .add_field(name='URL', value='[Click]({0.source.url})'.format(self))
                .set_thumbnail(url=self.source.thumbnail)
                .set_footer(text="First time? Do =help",icon_url="https://cdn.discordapp.com/emojis/874094506952241243.png?v=1")
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('你妈的有error还不快fix: {}'.format(str(error)))
    ...
    @commands.command(name='leave', aliases=['disconnect' , "lea" ])
    async def _leave(self, ctx: commands.Context):
        ...
        await ctx.send(":disconnect: Bot has been **successful disconnected!**")
    ...
    async def _pause(self, ctx: commands.Context):
        ...
        if ctx.voice_state.is_playing and ctx.voice_state.voice.is_playing():
            ...
            await ctx.message.add_reaction("⏸️")
            await ctx.send('⏸️ Music has been **pause.**')
    ...
    async def _resume(self, ctx: commands.Context):
        ...
        if ctx.voice_state.is_playing and ctx.voice_state.voice.is_paused():
            ...
            await ctx.message.add_reaction("▶️")
            await ctx.send('▶️ Music has been **resume.**')
    ...
    async def _stop(self, ctx: commands.Context):
        ...
        if ctx.voice_state.is_playing:
            ...
            await ctx.send("⏹ Music have been song and clear all queue")
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            await ctx.message.add_reaction('⏭')
            await ctx.send('⏭ Music have been skip.')
    ...
    async def _remove(self, ctx: commands.Context, index: int):
        ...
        await ctx.message.add_reaction("🪹")
        await ctx.send(' **Removes a song from the queue at a given index.**')
    ...
    async def _loop(self, ctx: commands.Context):
        ...
        await ctx.message.add_reaction("🔁")
        await ctx.send('🔁 Loop has been **Enable**!,If you want to **disable** write =loop again')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send(':loading: An error occurred while processing this request: {}'.format(str(e)))
            else:
                if not ctx.voice_state.voice:
                ...
                await ctx.send(':desktop: Searching 🔎 {}'.format(str(source)))
        await ctx.message.add_reaction('🔎')
